 ...
 */
#define bucket_size 4 //default 4

struct Bucket {
    TagType   tags[bucket_size];//4个tag,unsigned char
    char      notused[bucket_size];//default 4
    ValueType vals[bucket_size];//4个val,item结构体指针
...
void assoc2_init(const int hashtable_init) {
    ...
    buckets = (struct Bucket *)alloc(hashsize * sizeof(struct Bucket));
...
int       kick_count = 0;
uint64_t cuckoo_depth = 0;
uint64_t cuckoo_count = 0;
...
#ifndef MEMC3_ENABLE_TAG
            if (it == NULL)//如果找到最后tag相等却是NULL对象，就返回false
                return false;
#endif

            if (keycmp(key, ITEM_key(it), nkey)) {//tag对比后进一步对比key确实相等
                ...
                buckets[i].tags[j] = 0;//清空tag
                buckets[i].vals[j] = 0;//清空指针
                hash_items --;//item数减1
...
void assoc2_delete(const char *key, const size_t nkey, const uint32_t hv) {
    //i1,i2对应cuckoo hash的两个位置，直接用于bucket数组，即bucket[i1],bucket[i2]
    TagType tag = _tag_hash(hv);
    size_t   i1 = _index_hash(hv);
    size_t   i2 = _alt_index(i1, tag);
    size_t lock = _lock_index(i1, i2, tag);
    ...
    if (try_del(key, nkey, tag, i1, lock))//执行
        return;
