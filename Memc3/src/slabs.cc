...
static int do_slabs_newslab(const unsigned int id) {
    ...
    if ((mem_limit && mem_malloced + len > mem_limit && p->slabs > 0) ||
        ...
        // MEMCACHED_SLABS_SLABCLASS_ALLOCATE_FAILED(id);//0605
        return 0;
    }
...
#endif
    mem_malloced += len;
    // MEMCACHED_SLABS_SLABCLASS_ALLOCATE(id);//0605
...
static void *do_slabs_alloc(const size_t size, unsigned int id) {
    ...
    if (id < POWER_SMALLEST || id > power_largest) {
        // MEMCACHED_SLABS_ALLOCATE_FAILED(size, 0);//0605
        return NULL;
    }
    ...
    if (ret) {
        ...
        *
        * 类型:
        * 内容:如何解决memc3对memcached的套用？
        * 添加者:bwb
        * 时间:2023-06-05
        *
        ******************/
        // MEMCACHED_SLABS_ALLOCATE(size, id, p->size, ret);
        // printf("MEMCACHED_SLABS_ALLOCATE\n");//全是这个
    } else {
        // MEMCACHED_SLABS_ALLOCATE_FAILED(size, id);
        // printf("MEMCACHED_SLABS_ALLOCATE_FAILED\n");
...
static void do_slabs_free(void *ptr, const size_t size, unsigned int id) {
    ...
    // MEMCACHED_SLABS_FREE(size, id, ptr);//0605
    ...
    sl_it->prev = 0;
    sl_it->next = (_slabbed_item*)(p->slots);
...
static void do_slabs_stats(ADD_STAT add_stats, void *c) {
    ...
    // APPEND_STAT("active_slabs", "%d", total);0605
    // APPEND_STAT("total_malloced", "%llu", (unsigned long long)mem_malloced);0605
