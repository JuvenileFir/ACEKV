...
void sigint_handler(int sig) {
  ...
  printf("log_0 Load Factor:      %5lf %%\n\n", m_piekv->log_->log_segments_[0]->table_stats_->count * 40 * 100.0 / m_piekv->mempool_->max_log_size_);
  printf("log_1 Load Factor:      %5lf %%\n\n", m_piekv->log_->log_segments_[1]->table_stats_->count * 40 * 100.0 / m_piekv->mempool_->max_log_size_);
  printf("log_2 Load Factor:      %5lf %%\n\n", m_piekv->log_->log_segments_[2]->table_stats_->count * 40 * 100.0 / m_piekv->mempool_->max_log_size_);
  printf("log_3 Load Factor:      %5lf %%\n\n", m_piekv->log_->log_segments_[3]->table_stats_->count * 40 * 100.0 / m_piekv->mempool_->max_log_size_);
...
void port_init() {
  ...
  for (uint16_t eid = 0; eid < THREAD_NUM; eid++) {
...
int main(int argc, char *argv[]){
    ...
    while ((ch = getopt(argc, argv, "hca:f:l:t:")) != -1) {
        switch (ch) {
        case 'h':
            printf(
    ...
    #endif
    #ifdef CLEANUP
      printf("[INFO] 运行cleanup\n");
    #endif
    #ifdef RELOAD
      printf("[INFO] 运行reload\n");
    #endif
    
    #if defined(size64)
      printf("[INFO] reload kv size 64\n");
    #elif defined(size256)
      printf("[INFO] reload kv size 256\n");
    #elif defined(size512)
      printf("[INFO] reload kv size 512\n");
    #elif defined(size8)
      printf("[INFO] reload kv size 8\n");
    #endif
    ...
    printf("Total Index Entries:    %10zu\n\n", m_piekv->hashtable_->all_table_stats_->count);
    printf("indexs Load Factor:     %5lf %%\n\n", m_piekv->hashtable_->calc_load_factor());
    printf("log_0 Load Factor:      %5lf %%\n\n", m_piekv->log_->log_segments_[0]->table_stats_->count * 40 * 100.0 / init_log_size);
    printf("log_1 Load Factor:      %5lf %%\n\n", m_piekv->log_->log_segments_[1]->table_stats_->count * 40 * 100.0 / init_log_size);
    printf("log_2 Load Factor:      %5lf %%\n\n", m_piekv->log_->log_segments_[2]->table_stats_->count * 40 * 100.0 / init_log_size);
    printf("log_3 Load Factor:      %5lf %%\n\n", m_piekv->log_->log_segments_[3]->table_stats_->count * 40 * 100.0 / init_log_size);
    printf("Get Hit Rate:           %5lf%%\n\n", m_piekv->hashtable_->all_table_stats_->get_found * 100. / m_piekv->hashtable_->all_table_stats_->get_request);
    printf("Max Throughput:         %.2lf\n\n", m_piekv->max_tput_);
    printf("Calc Load Factor:       %5lf%%\n\n", m_piekv->hashtable_->calc_load_factor());
