  ...
  if (pktlen > (kMaxFrameLen - kResCounterLen - kMaxSetReturn - kEndMarkLen)) {
    ...
    bool ret = piekv_->set(t_id_, key_hash, ptr + sizeof(RxSet_Packet),
        ...
        if (piekv_->latencys[t_id_] < 1) {
            ...
            if (temp <= 10000) {
            printf("latency%ld: %ld  (set)\n", t_id_, temp);
            //     piekv_->log_->log_segments_[t_id_]->table_stats_->latency += temp;
                piekv_->latencys[t_id_]++;
            }
        }
    #endif
    ...
    // printf("[SET]type: %u, key_len: %u, keyhash_len: %u, val_len: %u\n", *(uint16_t *)ptr, rxset_packet->key_len, rxset_packet->key_hash_len, rxset_packet->val_len);
    ptr += sizeof(RxSet_Packet) + rxset_packet->key_len + 
           rxset_packet->key_hash_len + rxset_packet->val_len;//rxset_packet四值相加
    // printf("[SET]next type: %u\n", *(uint16_t *)ptr);
...
{   
    ...
    #ifdef LATENCY
        auto t2 = Clock::now();//计时结束
        if (piekv_->latencyg[t_id_] < 1) {
            // std::cout <<"latency"<<t_id_<<":"<<std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count()<< '\n';
            temp = std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count();
            if (temp <= 10000) {
                printf("latency%ld: %ld  (get)\n", t_id_, temp);
            //     piekv_->log_->log_segments_[t_id_]->table_stats_->latency += temp;
                piekv_->latencyg[t_id_]++;
            }
        }
    #endif
    ...
    // printf("[GET]type: %u, key_len: %u, keyhash_len: %u\n", *(uint16_t *)ptr, rxget_packet->key_len, rxget_packet->key_hash_len);
	ptr += sizeof(RxGet_Packet) + rxget_packet->key_len + rxget_packet->key_hash_len;
    // printf("[GET]next type: %u\n", *(uint16_t *)ptr);
...
void RTWorker::worker_proc() {
    ...
    while (piekv_->is_running_) {
        ...
        if (nb_rx > 0) m_piekv->hashtable_->all_table_stats_->rx_burst += 1;
            ...
            while (*(uint16_t *)ptr != MEGA_PKT_END) {
                ...
                if (*(uint16_t *)ptr == MEGA_JOB_GET) {
                    ...
                    *(uint16_t *)tx_ptr = THREAD_NUM;
                    ...
                    // printf("[INFO]出问题 ptr16:%u\n", *(uint16_t *)ptr);
                    // ptr += 2;
                    // rte_pktmbuf_dump(stdout, rx_buf[i], rx_buf[i]->pkt_len);//0628
                    // return;//0628
                    // continue;
                    break;
                }
