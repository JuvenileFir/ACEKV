...
###在server端运行，然后将参数传递到client端
    ...
    for idx in ${times[@]}; do
        ...
        nohup sudo -S sudo ./main_p8 -f 2 -l $logsize -t 30 -a $threshold > ../data/${time1}p${zu}${logsize}_${wratio}_ghr${idx}.log 2>&1 &
...
remotessh
        ...
        while true
        do
        serverpid=$(ps aux|grep "./main_p8"|grep -v "grep"|awk '{print $2}')
            if test -n "$serverpid"
            then
    ...
    for idx in ${times[@]}; do
        echo $wratio
        echo "start executing on server"
        nohup sudo -S sudo ./main_m8 -f 2 -l $logsize -t 30 > ../data/${time1}m${zu}${logsize}_${wratio}_ghr${idx}.log 2>&1 &
        ssh bwb@10.176.64.35 << remotessh
        cd ./Client/build/app/
...
remotessh
        ...
        while true
        do
        serverpid=$(ps aux|grep "./main_m8"|grep -v "grep"|awk '{print $2}')
            if test -n "$serverpid"
            then
                echo ">>>>server is running"
                sleep 2
