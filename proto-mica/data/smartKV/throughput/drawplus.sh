...
PY_DIR_U=$(ls |grep draw*u.py)
PY_DIR_Z=$(ls |grep draw*z.py)
...
then
    ...
    then
        ...
        file_names=$(ls $WORK_DIR |grep -oP '.*x(?=.csv)')
        python3 $PY_DIR_Z ${WORK_DIR} 1 ${file_names[@]} # 3
        file_names=$(ls $WORK_DIR |grep '.csv')
        for file in $file_names; do
    ...
    then
        ...
        file_names=$(ls $WORK_DIR |grep -oP '.*x(?=.csv)')
        python3 $PY_DIR_U ${WORK_DIR} 1 ${file_names[@]} # 3
        file_names=$(ls $WORK_DIR |grep '.csv')
        for file in $file_names; do
            mv $file ./tput_res_files/
        done
    ...
    then
        ...
        python3 $PY_DIR_U $WORK_DIR/tput_res_files 1 ${file_names[@]} # 3
    elif [ $1 == "-3z" ]
    then
        echo 将重画图3
        file_names=$(ls -r $WORK_DIR/tput_res_files |grep -oP '.*x(?=.csv)')
        python3 $PY_DIR_Z $WORK_DIR/tput_res_files 1 ${file_names[@]} # 3
    elif [ $1 == "-a" ]
    then
        echo 将重画图0-3
        file_names=$(ls $WORK_DIR/hit_res_files |grep -oP '.*a(?=.csv)')
