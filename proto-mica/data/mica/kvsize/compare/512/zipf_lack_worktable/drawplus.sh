...
then
    ...
    then
        ...
        # 获取当前目录下所有文件名
        file_names=$(ls -r $WORK_DIR |grep -oP '.*a(?=.csv)')
        python3 $PY_DIR ${WORK_DIR} 2 ${file_names[@]} # 0
        file_names=$(ls -r $WORK_DIR |grep -oP '.*x(?=.csv)')
        python3 $PY_DIR ${WORK_DIR} 0 ${file_names[@]} # 1
        file_names=$(ls -r $WORK_DIR |grep -oP '.*t(?=.csv)')
    ...
    then
        echo 将重画图0
        file_names=$(ls -r  $WORK_DIR/hit_res_files |grep -oP '.*a(?=.csv)')
        python3 $PY_DIR $WORK_DIR/hit_res_files 2 ${file_names[@]} # 0
    elif [ $1 == "-1" ]
    then
        echo 将重画图1
        file_names=$(ls -r  $WORK_DIR/hit_res_files |grep -oP '.*x(?=.csv)')
    ...
    then
        echo 将重画图2
        file_names=$(ls -r  $WORK_DIR/hit_res_files |grep -oP '.*t(?=.csv)')
        python3 $PY_DIR $WORK_DIR/hit_res_files 3 ${file_names[@]} # 2
    elif [ $1 == "-3" ]
    then
        echo 将重画图3
        file_names=$(ls -r  $WORK_DIR/tput_res_files |grep -oP '.*x(?=.csv)')
        python3 $PY_DIR $WORK_DIR/tput_res_files 1 ${file_names[@]} # 3
    elif [ $1 == "-4" ]
    then
        echo 将重画图4
        file_names=$(ls -r  $WORK_DIR/tput_res_files |grep -oP '.*t(?=.csv)')
        python3 $PY_DIR $WORK_DIR/tput_res_files 4 ${file_names[@]} # 4
    elif [ $1 == "clean" ]
    then
        rm -f  ./hit_res_files/* ./tput_res_files/*
    else
        echo 请输入正确参数
    fi
else
