...
then
    ...
    then
        echo 将重画图0
        file_names=$(ls -t $WORK_DIR/hit_res_files |grep -oP '.*a(?=.csv)')
        python3 $PY_DIR $WORK_DIR/hit_res_files 2 ${file_names[@]} # 0
    elif [ $1 == "-1" ]
    then
        echo 将重画图1
        file_names=$(ls -t $WORK_DIR/hit_res_files |grep -oP '.*x(?=.csv)')
    ...
    then
        echo 将重画图2
        file_names=$(ls -t $WORK_DIR/hit_res_files |grep -oP '.*t(?=.csv)')
        python3 $PY_DIR $WORK_DIR/hit_res_files 3 ${file_names[@]} # 2
    elif [ $1 == "-3" ]
    then
        echo 将重画图3
        file_names=$(ls -t $WORK_DIR/tput_res_files |grep -oP '.*x(?=.csv)')
        python3 $PY_DIR $WORK_DIR/tput_res_files 1 ${file_names[@]} # 3
    elif [ $1 == "-4" ]
    then
        echo 将重画图4
        file_names=$(ls -t $WORK_DIR/tput_res_files |grep -oP '.*t(?=.csv)')
