...
thresholds=(50) #跑tput
kvsizes=(8 64 256 512) # 8 64 256 512
...
for kvsize in ${kvsizes[@]}; do
    ...
    for idx in ${times[@]}; do
        ...
        nohup sudo -S sudo ./main_p${kvsize} -f 2 -l $logsize -t 30 -a $threshold > ../data/${time1}p${zu}_log${logsize}_kv${kvsize}_tput_nr${idx}.log 2>&1 & #for tput
...
#         echo "running mica"
#         echo "start executing on server"
#         nohup sudo -S sudo ./main_m${kvsize} -f 2 -l $logsize -t 30 > ../data/${time1}m${zu}_log${logsize}_kv${kvsize}_tput_nr${idx}.log 2>&1 & #for tput
#         ssh bwb@10.176.64.35 << remotessh
#         cd ./Client/build/app/
