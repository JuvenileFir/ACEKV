...
def getiLF(logfile,csvfile):
    ...
    with open(logfile,"r") as f:
        ...
        for line in lines:
            words = re.split(r"[ ]+", line)
            if words[0] == "[P]indexs": # [P]indexs / full
                used_time = float(words[3])
                Y.append(used_time)
                if used_time < 5 or (used_time - AX) > AX_thres:
                    A.append(1)
                    AX = used_time
                else:
                    A.append(0)
...
def getmhr(logfile,csvfile):
    ...
    with open(logfile,"r") as f:
        ...
        for line in lines:
            words = re.split(r"[ ]+", line)
            if words[0] == "[P]mica" and words[1] == "evict-valid:": #可以改invalid
...
def getmetl(logfile,csvfile):
    ...
    with open(logfile,"r") as f:
        ...
        for line in lines:
            words = re.split(r"[ ]+", line)
            if words[0] == "[P]mica" and words[1] == "evict-invalid-ps:": #"evict-invalid-ps:":
                used_time = float(words[2])
                Y.append(used_time)
    # _csvfile = "./tput/tput_" + csvfile
    # savesingleToCSV(_csvfile, Y) 
...
csvdir = sys.argv[2] + "/"
if sys.argv[1] == '1':
    logdir = csvdir + "hitlog/"
elif sys.argv[1] == '0':
    logdir = csvdir + "tputlog/"
    refine_all = False
elif sys.argv[1] == '2':
    logdir = csvdir + "hitlog/"
    mica_evict_hit = True
...
for argva in sys.argv[3:-2]:
    logfiles.append(logdir + argva + ".log")
    csvfiles.append(csvdir + argva + "_a.csv")
    csvfileT.append(csvdir + argva + "_t.csv")
    csvfileX.append(csvdir + argva + "_x.csv")
...
for i in range(0,len(logfiles)):
    ...
    if mica_evict_hit:
        Y = getmhr(logfiles[i],csvfiles[i])
    else:
        Y = getghr(logfiles[i],csvfiles[i])
    if mica_evict_ttl:
        P = getmetl(logfiles[i],csvfiles[i])
    else:
    ...
    if refine_all:
        saverefinedquadrToCSV(csvfileT[i], T, X, Y, P, T_allow)
    saverefinedquadrToCSV(csvfileX[i], T, X, Y, P, X_allow)
