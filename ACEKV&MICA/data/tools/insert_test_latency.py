...
def getltc(logfile,csvfile):
    ...
    with open(logfile,"r") as f:
        ...
        for line in lines:
            words = re.split(r"[ ]+", line)
            if words[0] == "[P]set" and words[1] == "latency:":
                used_time = int(words[2])
                Y.append(used_time)
    # _csvfile = "./tput/tput_" + csvfile
    # savesingleToCSV(_csvfile, Y)
    return Y
def getlatency(logfile,csvfile):
    Y = []
    with open(logfile,"r") as f:
        data = f.read()
        lines = data.split("\n")
        for line in lines:
            words = re.split(r"[ ]+", line)
            if words[0] == "latency0:" and int(words[1]) < 2000: #memc3的话一个线程就够了
                used_time = int(words[1])
                Y.append(used_time)
            # if words[0] == "latency1:" and int(words[1]) < 2000:
            #     used_time = int(words[1])
            #     Y.append(used_time)
            # if words[0] == "latency2:" and int(words[1]) < 2000:
            #     used_time = int(words[1])
            #     Y.append(used_time)
            # if words[0] == "latency3:" and int(words[1]) < 2000:
            #     used_time = int(words[1])
            #     Y.append(used_time)
    # _csvfile = "./tput/tput_" + csvfile
    # savesingleToCSV(_csvfile, Y)
...
for i in range(0,len(logfiles)):
    # X = getltc(logfiles[i],csvfiles[i])
    X = getlatency(logfiles[i],csvfiles[i])
