...
def draw_insert_new(idx):
    ...
    for i in range(0,len(Xs)):
        plt.plot(Xs[i],Ys[i],linewidth =4.0,label=labels[i],linestyle=linestyles[i],marker=markers[i],markersize=12,color=colors[i],markerfacecolor='white',markeredgewidth=5) # markerfacecolor='white'可画空心
    temp_limit = 100
    if MY_define == 70 and (idx == 0 or idx == 3):
        temp_limit = 70
    xy_limit = [200000000, 100, temp_limit, 4000000]
    ...
    ax.set_xlabel('$\mathrm{Load\ Factor\ (' + xylabel_units[axtype[0]] + ')}$') 
    ax.set_ylabel('$\mathrm{Throughput\ (' + xylabel_units[axtype[1]] + ')}$')
    # ax.invert_yaxis()
    ax.legend(prop=font_times, bbox_to_anchor=(0.45,-0.12),loc = 9, ncol = 3, frameon=False) # 图例
    plt.subplots_adjust(left=0.15, right=0.90, top=0.98, bottom=0.25)
