...
def savequadrToCSV(csvfile, T, X, Y, A):
    with open(csvfile,"w") as f:
        for i in range(0,len(X)):
            if A[i] == 1:
                t = T[i]
    ...
    with open(logfile,"r") as f:
        ...
        for line in lines:
            ...
            if words[0] == "[P]indexs": # [P]index / indexs
                used_time = float(words[3])
                Y.append(used_time)
                if used_time < 3 or (used_time - AX) > 5:
                    A.append(1)
                    AX = used_time
                else:
                    A.append(0)
...
def getfull(logfile,csvfile):
    ...
    with open(logfile,"r") as f:
        ...
        for line in lines:
            words = re.split(r"[ ]+", line)
            if words[0] == "[P]total" and words[2] == "path:": # perset percukoo
                used_time = float(words[3])
                Y.append(used_time)
    # _csvfile = "./ghr/ghr_" + csvfile
    # savesingleToCSV(_csvfile, Y)
    return Y
def getfullandexpire(logfile,csvfile):
    Y = []
    with open(logfile,"r") as f:
        data = f.read()
        lines = data.split("\n")
        for line in lines:
            words = re.split(r"[ ]+", line)
            if words[0] == "[P]full" and words[1] == "expirted":
                used_time = float(words[3])
                if used_time > 100.0:
                    used_time = 100.0
                Y.append(used_time)
...
for i in range(0,len(logfiles)):
    ...
    Y = getfull(logfiles[i],csvfiles[i])
    # P = getfullandexpire(logfiles[i],csvfiles[i])
    ...
    savequadrToCSV(csvfiles[i], T, X, Y, X_allow)
