...
static UNUSED void mehcached_zipf_init_copy(struct zipf_gen_state *state, const struct zipf_gen_state *src_state,
                                     uint64_t rand_seed) {
  assert(rand_seed < (1UL << 48));
...
static UNUSED void mehcached_zipf_change_n(struct zipf_gen_state *state, uint64_t n) { state->n = n; }
...
static UNUSED void mehcached_test_zipf(double theta) {
  double zetan = 0.;
