...
LogSegment::LogSegment(MemPool *mempool, int index) {
    ...
    seg_log_size_ = mempool_->max_log_size_;//1<<31 2G
    
    seg_offset_mask_ = kSegOffset - 1;//每个segment分得的offset部分的mask,为(1<<46) - 1
...
Log::Log(MemPool *mempool) {
    ...
    max_size_ = mempool_->max_log_size_ * segment_num_;// 2G * 4 = 8G
...
bool LogSegment::is_valid(uint64_t log_offset) {
    ...
    // return distance(log_offset) <= static_cast<uint64_t>(seg_log_size_ * 2.0 / 3); //就改这一行1108
    return distance(log_offset) <= static_cast<uint64_t>(seg_log_size_); //改动之前1219
}

uint64_t LogSegment::distance(uint64_t log_offset) {
    return ((tail_ - log_offset + seg_offset_mask_ + 1) & seg_offset_mask_); 
}
...
uint64_t LogSegment::AllocItem(uint64_t item_size, bool compensator) {//实际上执行的是MICA的push_tail()功能

    // uint64_t item_size = mem_size;
    assert(item_size == ROUNDUP8(item_size));

    if (__glibc_likely(item_size <= mempool_->max_log_size_)) {//肯定过
        uint64_t offset = tail_;
        uint64_t v = tail_ + item_size;
       ...
       //先注释掉offset回环的事，其实根本不需要考虑0619
        ...
        while (v > head_ + seg_log_size_) {
            this->pop_head();//确保head至tail的范围不超过log_size
            if (compensator) table_stats_->count -= 1;
        }
...
void LogSegment::pop_head() {
    ...
    head_ += item->item_size;
    // if (head_ >= dynamic_end_) head_ = segment_idx_ * kSegOffset;//先注释掉offset回环的事，其实根本不需要考虑0619
}
...
//   printf("valid count:            %10zu\n\n", table_stats_->count);
  printf("set request:            %10zu | ", table_stats_->set_count);
  printf("set_success:            %10zu\n", table_stats_->set_success);
  printf("set_fail:               %10zu | ", table_stats_->set_fail);
  printf("set_key_duplicate:      %10zu | ", table_stats_->set_overwrite_key_duplicate);
  printf("set_mica_evicted:       %10zu\n", table_stats_->set_mica_evicted);
  ...
  printf("clean up scan:          %10zu | ", table_stats_->cleanup_scan);
  printf("clean up success:       %10zu | ", table_stats_->cleanup_succ);
