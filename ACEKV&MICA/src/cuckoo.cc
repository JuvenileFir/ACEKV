...
static const uint8_t MAX_BFS_PATH_LEN = 10;
...
void unlock_two_buckets(Bucket *ptr, twoBucket twobuckets) {
  ...
  if (twobuckets.b1 != twobuckets.b2) write_unlock_bucket(&ptr[twobuckets.b2]);
}
...
cuckooStatus run_cuckoo(Bucket *ptr, tablePosition* tp, int *cuckoo_depth, int *break_loop, LogSegment *seg) {
  ...
  *
  ******************/
  write_unlock_bucket(&ptr[tp->bucket]);//这里有必要先把备选bkt - ptr[tp->bucket] 解锁吗？ 有!!! 必须解锁!!!
  struct cuckooRecord cuckoopath[MAX_BFS_PATH_LEN];
  //用tag算出另一个bucket,以此启动
  uint32_t new_bucket = alt_bucket(tp->bucket, TAG((ptr[tp->bucket]).item_vec[tp->slot]));
  
  bool done = false, random_pick = false;
  int retry_count = 0;

  while (!done) {
    int depth = cuckoopath_search(ptr, cuckoopath, new_bucket, random_pick, seg);//实现了BFS路径寻找及记录，超过最长路径直接返回-1
    /******************
    *
    * 类型:修改
    * 内容:修改了超过cuckoo path上限直接随机踢而不返回错误的功能
    *     因为已经选定了最小counter对应的slot，因此随机踢掉其备选bucket中的随机位置即可
    * 添加者:bwb
    * 时间:2023-10-28
    *
    ******************/
    // int depth = cuckoopath_search(ptr, cuckoopath, tb.b1, tb.b2);//老版本cp_search
    if (retry_count/* ++ */ > 10000 || depth < 0) {
      if (retry_count > 10000) *(break_loop) += 1;
      *cuckoo_depth = MAX_BFS_PATH_LEN;//即使随机踢，仍然将path按照上限计算赋值
      // ptr[new_bucket].item_vec[random() % slot_num] = ptr[tp->bucket].item_vec[tp->slot];
      tp->slot = random() % slot_num;
      write_lock_bucket(&ptr[tp->bucket]);
  ...
  // if (!done) write_unlock_bucket(&ptr[tp->bucket]); //这里的解锁统一放在set里面处理了
  return done ? ok : failure;
}
...
int cuckoopath_search(Bucket *ptr, cuckooRecord *cuckoopath, const uint32_t b1, const uint32_t b2) {
  ...
  for (int i = x.depth; i >= 0; i--) {
    cuckoopath[i].slot = x.pathcode % slot_num;
    x.pathcode /= slot_num;
  }
...
int cuckoopath_search(Bucket *ptr, cuckooRecord *cuckoopath, const uint32_t new_bucket, bool random_pick, LogSegment *seg) {
  bSlot x = slot_search(ptr, new_bucket, random_pick, seg);
  ...
  for (int i = 1; i <= x.depth; ++i) {
    ...
    Bucket *b = &ptr[curr->bucket];
    // write_lock_bucket(b);
    //下面的if块作用：如果有空位，直接输出（大概率是既定位置，也可能是路上中途发现空位（几乎不可能））
    if (is_slot_empty(b->item_vec[curr->slot])) {
      // write_unlock_bucket(b);
      return i;
    }
...
bSlot slot_search(Bucket *ptr, const uint32_t b1, const uint32_t b2) {
  ...
  while (!empty(que)) {
    ...
    uint32_t startingslot = x.pathcode % slot_num;
    for (uint32_t i = 0; i < slot_num; ++i) {
      uint32_t slot = (startingslot + i) % slot_num;
      if (is_slot_empty(b->item_vec[slot])) {
        // We can terminate the search here
        x.pathcode = x.pathcode * slot_num + slot;
        write_unlock_bucket(b);
        return x;
      }
      ...
      if (x.depth < MAX_BFS_PATH_LEN - 1) {
        ...
        if (alt_bucket_index != b1 && alt_bucket_index != b2) {
          bSlot y = {alt_bucket_index,
                     (uint16_t)(x.pathcode * slot_num + slot), (int8_t)(x.depth + 1)};
          enqueue(&que, y);
        }
      }
    }
...
bSlot slot_search(Bucket *ptr, const uint32_t new_bucket, bool random_pick, LogSegment *seg) {
  ...
  que.first_ = 0;
  que.last_ = 0;
  uint64_t min = 254,  min_slot, submin = 255;
  int64_t min_i = 0, submin_i = -1;
  // The initial pathcode informs cuckoopath_search which bucket the path starts on
  ...
  while (!empty(que)) {
    ...
    min_i = 0;
    min_slot = slot_num;
    // write_lock_bucket(b);//卡在这里了？（如果加上slot的锁，就会卡在这里）
    ...
    for (uint32_t i = 0; i < slot_num; ++i) {
      ...
      if (is_slot_empty(b->item_vec[slot])) {
        ...
        // write_unlock_bucket(b);
        return x;
      } else if (!seg->is_valid(ITEM_OFFSET(b->item_vec[slot]))) {
        x.pathcode = x.pathcode * slot_num + slot;
        return x;
      }
    #ifdef USE_COUNTER
      else if (COUNTER(b->item_vec[slot]) < min) {
        submin = min;//其实不需要记录倒数第二小的count值
        min = COUNTER(b->item_vec[slot]);
        submin_i = min_i;
        min_i = i;
      }
    #else
      min_i = random() % slot_num;
    #endif
    }
      // 只要没达到最大深度，就继续向深层遍历，创建新bSlot入队
      if (x.depth < MAX_BFS_PATH_LEN - 1) {
        if (random_pick && submin_i != -1) min_i =  random() % 2 == 0 ? submin_i : min_i;
        min_slot = (startingslot + min_i) % slot_num;
        assert(!full(que));
        uint32_t alt_bucket_index = alt_bucket(x.bucket, TAG(b->item_vec[min_slot]));
        if (alt_bucket_index != new_bucket) {
          bSlot y = {alt_bucket_index, (uint16_t)(x.pathcode * slot_num + min_slot),
                     (int8_t)(x.depth + 1)};
          enqueue(&que, y);
        }
      }
...
/* uint32_t cuckoopath_move(Bucket *ptr, cuckooRecord *cuckoopath, int depth, twoBucket *tb) {
  ...
  while (depth > 0) {
    ...
    uint64_t v = from_b->item_vec[fs];
    #ifdef RUN_MICA
      to_b->item_vec[ts] = v;//核心操作：item_vector迁移
    #else
      to_b->item_vec[ts] = ITEM_VEC(TAG(v), COUNTER(v) + 2, ITEM_OFFSET(v));//核心操作：item_vector迁移
    #endif
...
uint32_t cuckoopath_move(Bucket *ptr, cuckooRecord *cuckoopath, int depth, const uint32_t new_bucket) {
  ...
  while (depth > 0) {
    ...
    #else
      to_b->item_vec[ts] = ITEM_VEC(TAG(v), COUNTER(v) + 3, ITEM_OFFSET(v));
    #endif
    from_b->item_vec[fs] = 0UL;//核心操作：倒二改成0，准备下一轮的迁移
    unlock_two_buckets(ptr, twob);
    depth--;
